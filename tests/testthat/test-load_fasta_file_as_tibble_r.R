test_that("SARS-CoV-2", {
  ...
  t <- load_fasta_file_as_tibble_r(fasta_filename)
...
test_that("fasta file -> tibble -> fasta file", {
  ...
  t <- load_fasta_file_as_tibble_r(fasta_filename_1)
...
test_that("tibble with empty sequences -> FASTA file -> tibble", {
  ...
  t_again <- load_fasta_file_as_tibble_r(fasta_filename = fasta_filename)
